...
(() => {
  ...
  function copyToClipboard(text) {
    ...
    function init() {
      ...
      // Dernier trigger cliqué (sert à l'animation fly-in / fly-out)
      ...
      function open(fromEl) {
        ...
        // Prépare l'état initial : dialog centré mais "vient" du bouton
        if (!reduce) {
          gsap.set(dialog, {
          ...
          gsap.set(inner, { rotateY: 0, rotateX: 0, transformOrigin: "50% 50%" });
          gsap.set(overlay, { opacity: 0 });
        } else {
          gsap.set(dialog, { x: 0, y: 0, scale: 1, rotateX: 0, rotateY: 0, rotateZ: 0, z: 0, filter: "blur(0px)" });
          gsap.set(inner, { rotateY: 0, rotateX: 0 });
        ...
        if (reduce) {
          ...
          const loader2 = overlay.querySelector(".reserve-front-loader");
          if (loader2) {
            tl.to(loader2, { opacity: 0, y: -4, duration: 0.25, ease: "power2.out" }, FLIP_START_AT - 0.15);
          }
      ...
      triggers.forEach((triggerEl) => {
        triggerEl.addEventListener("click", (e) => {
          // stop le scroll smooth / autres handlers
          e.preventDefault();
          e.stopPropagation();
